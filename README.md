...
A small declarative language for describing patterns in sets of events.
...
Before describing the PQL language, it is important to understand what is meant when we talk about 'events'.

In this context, events are immutable records of things that occur in the world or in our application.  Instead of storing the current state of objects in a problem domain, we can use events to store their entire history.  From the history, we can reacreate the state of the objects at any point in time and audit the state changes that brought them there.
...
We typically work with ordered sets of events.  These can be created including events related to a specific object, occurring during certain window of time, or at a certain location.
...
The PQL language consists of *blocks* of *matching statements*.  Matching statements describe specific patterns of events in a set and Blocks combine multiple matching statements to describe more complex patterns.

When applied to a set of events, matching statements produce a set of *matches*, each consisting of zero or more events.  A statement is said to succeed in matching a set if its set of matches has at least one member, and fail if its set of matches is null.
...
For example, if we had a set of events related to final exam scores and wanted to find the highest score in each course, we could use the following matching expression:
...
There are three types of selective subexpressions:
  ...
  - **Limiting Expressions:** limiting expressions select subsets of events within each match without changing the overall number of matches.
    ...
    - `FIRST` / `FIRST n` select events from the beginning of the set
    - `LAST` / `LAST n` select events from the end of the set
  ...
  - **Cardinality Expressions:** cardinality expressions determine the number of matches.
    ...
    - `EACH` matches once for each event in the set, selecting one event in each match.
    - `ANY` always matches one time, whether or not any events in the set meet the conditions.  If any events do meet the conditions, it selects them, otherwise it selects no events.
    - `NONE` matches one time and selects no events.  It matches only if no events in the set meet the conditions.
...
Values can be references to the value of a column on the event currently being considered (the *subject event*), literal values, or references to values of other events in the set (*value expressions*).
...
- **Literals:**  literals express a value directly without reference to the subject event or other  events in the set.
...
- **Value Expressions:** value expressions are similar to filter expressions, but are used to compare the subject event to other events in the set.  An example expression would look like `(MAX time WHERE status = 'error')`. Value expressions are always surrounded by `()` and consist of the following parts in order.
...
Blocks match a set ONLY if all of their constituent matching statements successfully match the set at least one time.  A block's matches are the cartesian product of the matches from each individual statement.  This gives the block a cardinality equal to the product of the cardinality of each of its constituent matching statements.

For example, If the preceding block was applied to following set, each statement would match 2 times for a cardinality of 2, and the block would match four times for a cardinality of four.
...
Matching statements with joins that successfuly match a set have no effect on the cardinality of a block.  Statements that fail to match have a cardinality of zero.

When applied to the following set of events, the preceding block will match only two times, selecting the sets of events with ids (1,3) and (2,4).
